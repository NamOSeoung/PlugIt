...
class SignUpPhoneVC: UIViewController {
    ...
    func delegateInit() {
        ...
        //텍스트 필드 입력 시 이벤트 발생
        phoneTF.addTarget(self, action: #selector(textFieldDidChange(textField:)), for: .editingChanged)
    }
    ...
    @IBAction func nextStepBtn(_ sender: Any) {
        let phoneCertVC = certificationStoryBoard.instantiateViewController(withIdentifier: "PhoneCertVC") as! PhoneCertVC
        self.present(phoneCertVC, animated: true, completion: nil)
    }
